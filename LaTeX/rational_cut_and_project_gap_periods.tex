...
(existence: Corollary~\ref{cor:inverse}).  Throughout, we identify
...
For irrational \(a\) and \(\omega > 0\), the strip has bounded
internal-coordinate width \(W=[-a\omega,\omega]\); since the internal
projection \(s\) is injective on \(\mathbb{Z}^2\) (by the same irrationality
argument used for \(\tilde p\) below) and \(\tilde p\) restricted to the
strip has finite fibres over any bounded interval --- the preimage of such
an interval intersected with the strip is a bounded parallelogram in
\(\mathbb{R}^2\) and hence contains only finitely many lattice points ---
the projected set \(\tilde p(A)\) is locally finite.

Unboundedness in both directions follows from a lattice-translate
construction whose quantor structure is the natural one for a
...
Proposition~\ref{prop:irrational} below), for every \(\varepsilon > 0\)
there exists \((p_0, q_0) \in \mathbb{Z}^2\) with
\(0 < |s(p_0, q_0)| < \varepsilon\).  For \(\varepsilon < 1\), the
relation \(s(p_0, q_0) = q_0 - a p_0\) with \(|s(p_0, q_0)| < 1\)
forces \(p_0 \ne 0\) (else \(|q_0| < 1\) would give \(q_0 = 0\),
contradicting \(|s(p_0, q_0)| > 0\)).  Hence \(|p_0| \ge 1\), and the
identity
\[
    \tilde p(p_0, q_0) \;=\; (1 + a^2)\, p_0 + a\, s(p_0, q_0)
\]
yields the uniform lower bound
\(|\tilde p(p_0, q_0)| \ge c_0 := 1 + a^2 - |a|\), which is strictly
positive since
\(c_0 = (|a| - \tfrac12)^2 + \tfrac34 \ge \tfrac34\).  Replacing
\((p_0, q_0)\) by \((-p_0, -q_0)\) if necessary, we may assume
\(\tilde p(p_0, q_0) \ge c_0\).

Now fix the target \(M\).  Setting
\(K := \lceil M / \tilde p(p_0, q_0) \rceil + 1\), the bound
\(\tilde p(p_0, q_0) \ge c_0 > 0\) gives the explicit estimate
\(K \le |M|/c_0 + 2\); choosing
\(\varepsilon \le c_0 \min(a\omega, \omega) / (|M| + 2 c_0 + 1)\)
(still positive, depending on \(M\)) therefore ensures
\(K \cdot |s(p_0, q_0)| < K \varepsilon \le \min(a\omega, \omega)\).
Then \(s(K p_0, K q_0) = K \cdot s(p_0, q_0) \in W\), so
\((K p_0, K q_0) \in A\), and
\(\tilde p(K p_0, K q_0) = K \cdot \tilde p(p_0, q_0) > M\).  The
negative direction is symmetric: apply the construction to \(-M\) and
negate the resulting lattice point.  The gap sequence is therefore
well defined via the increasing \(\mathbb{Z}\)-indexed enumeration of
\(\tilde p(A)\).  (The same construction, with explicit constants, is
used in the Lean formalisation to discharge \texttt{NoMaxOrder} and
...
simplifies the paper's argument at Step~3: instead of assuming both
forward and backward invariance \(A + v = A\), the Lean step
\texttt{lattice\_translation\_must\_be\_zero} requires only the
forward inclusion \(A + v \subseteq A\) -- the contradiction near
either endpoint of the window \(W\) is derivable from the forward
direction alone.  Correspondingly, Step~2 in
\texttt{period\_lifts\_to\_lattice\_translation} produces only forward
invariance.
...
\begin{itemize}
    ...
    \item Theorem~\ref{thm:main} is established for the geometric
          enumeration in the headline statement
          \[
...
geometric projection is fully threaded through to the period theorems on
