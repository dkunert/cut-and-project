...
Indeed, \(x_0\in H\) while \(x_0-1\notin H\), so \(x_0\) is the unique element
of \(H\) whose predecessor is not in \(H\).  If \(H+\tau=H\), then
\(x_0+\tau\) must have the same property and hence must equal \(x_0\).
Thus \(\tau\equiv0\pmod D\).
...
\label{ex:set_gt}
For \(\alpha = 3\), \(\beta = 4\), \(\omega = 4\): \(N = 12 + 16 + 1 = 29\)
and \(D = 25\), so \(N > D\) and \(D \nmid N\).  Theorem~\ref{thm:main}
gives \(\lambda_{\mathrm{multiset}} = 29\), while
Theorem~\ref{thm:setperiod} gives \(\lambda_{\mathrm{set}} = 1\).  Direct
computation confirms both values.
\end{example}
