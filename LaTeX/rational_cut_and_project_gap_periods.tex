...
\title{Period Formulas for Rational\\ Cut-and-Project Strip Projections:\\ Multiset and Set Cases}
\author{
...
residue parametrisation; its only translational symmetry is the identity,
...
when \(N \le D\); when \(N > D\) the set-valued period collapses to~\(1\),
because every integer is then a projected value.
...
form a proper cyclic interval in the natural residue parametrisation; its
...
Both proofs have been formalised in Lean~4 with the geometric residue map
\(c_r \equiv -\alpha\beta^{-1}r \pmod{D}\) threaded through to the period
theorems on both the multiset and set sides.  The formalisation verifies
the residue-class and minimal-period arguments; the strip-to-residue
reduction itself is hand-verified in Section~\ref{sec:cnp} and forms the
boundary between the two layers documented in
Section~\ref{subsec:lean_scope}.
...
The minimality argument relies on the following self-contained fact
about cyclic intervals; we state it separately for reuse and clarity.

\begin{lemma}[Trivial stabiliser of a cyclic interval]
...
the minimal period \(\lambda\) divides \(N\).
...
Now suppose, for contradiction, that the minimal period \(\lambda'\) satisfies
\(\lambda' < N\).  Since \(N\) is a period, the minimal period divides \(N\).
Thus we may write
\[
    N = k\lambda'
\]
...
\]
By \(\lambda'\)-periodicity, every block of \(\lambda'\) consecutive
gaps has the same sum \(\sigma\); that is, for every
\(i \in \mathbb{Z}\),
\[
    \delta^{(i)} + \delta^{(i+1)} + \cdots + \delta^{(i+\lambda'-1)}
    = \sigma.
\]
Hence
\[
...
\]
for all \(i\).  Since \(i \mapsto i+\lambda'\) is a bijection of
...
Sort the underlying set
\(\bigl\{\tilde{p} \,:\, \tilde{p}\text{ occurs as a projected value}\bigr\}
\subseteq \mathbb{Z}\) in increasing order, with each value listed once.
...
Both period theorems have been formalised in
...
\texttt{set\_main\_theorem\_geometric\_concrete}).  The formalisation
...
Lemma~\ref{lem:cyclic_interval_stabilizer} (trivial stabiliser)
...
\hline
\multicolumn{3}{l}{\textit{Multiplier-aware infrastructure (geometric residue map \(c_r \equiv -\alpha\beta^{-1}r \pmod{D}\))}} \\
\hline
...
The formalisation is structured in two layers, distinguishing what is
...
(Section~\ref{sec:cnp}).} The geometric content of the cut-and-project
construction --- the strip in~\(\mathbb{R}^2\), the projection onto
\(y = (\alpha/\beta)x\), and the derivation of the residue formula
\(c_r \equiv -\alpha\beta^{-1}r \pmod{D}\) in
equation~\eqref{eq:residue} --- is verified by hand in Section~\ref{sec:cnp}.
...
multiplier (Sections~\ref{sec:residue}--\ref{sec:setvalued}).} Starting
from the residue model, the Lean formalisation \emph{includes the
...
\begin{itemize}
    \item Coprimality, residue distribution (uniform and non-uniform), the
          cyclic interval structure, the trivial stabiliser lemma, the
          generic minimality argument, the degenerate case, and the case
          dispatch for both the multiset and set conventions are formalised
...
mathematics; what is novel in the formalisation is that Layer~2 closes
the residual gap left by previous formalisations, in which the abstract
typeclass \texttt{GeometricProjection} was instantiated only for the
normalised residue map (\(u = 1\)).  After Phase~2 of the multiplier
...
exploration, drafting, proof construction, and the creation of the Lean formalisation.
