...
construction.  Density of
\(s(\mathbb{Z}^2)=\{n - a m : m, n \in \mathbb{Z}\}\) in \(\mathbb{R}\)
(see Step~3 of the proof of Proposition~\ref{prop:irrational} below)
yields a pair \((p_0, q_0) \in \mathbb{Z}^2\) with \(0 < |s(p_0, q_0)|\)
arbitrarily small.  Since \(\tilde p\) is injective on \(\mathbb{Z}^2\)
(Step~1 below) and \((p_0, q_0)\ne(0,0)\), we have
\(\tilde p(p_0, q_0) \ne 0\); replacing \((p_0, q_0)\) by
\((-p_0, -q_0)\) if needed, we may assume \(\tilde p(p_0, q_0) > 0\).
For any \(K \in \mathbb{Z}\),
\[
    s(K p_0, K q_0) \;=\; K \cdot s(p_0, q_0),
    \qquad
    \tilde p(K p_0, K q_0) \;=\; K \cdot \tilde p(p_0, q_0),
\]
so by choosing \(|s(p_0, q_0)|\) small enough that
\(|K \cdot s(p_0, q_0)| \le \min(a\omega, \omega)\) for the desired
range of \(K\), the lattice point \((K p_0, K q_0)\) lies in \(A\), and
\(\tilde p(K p_0, K q_0) \to \pm\infty\) as \(K \to \pm\infty\).  The
gap sequence is therefore well defined via the increasing
