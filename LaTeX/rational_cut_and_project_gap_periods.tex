...
The formalisation comprises approximately 1{,}800 lines of verified code
...
The formalised proof covers the algebraic and combinatorial core for both
periods: coprimality, residue distribution (uniform and non-uniform),
cyclic interval structure, the trivial stabiliser lemma, the generic
minimality argument, the degenerate case, and the case dispatch for both
the multiset and set conventions.
...
flattened to \(\{0, 1\}\) by an indicator function: \texttt{set\_V} replaces
\texttt{V}, \texttt{set\_size} replaces \(N\), and the resulting bi-infinite
\texttt{set\_difference\_sequence} discharges the abstract hypotheses of
\texttt{set\_main\_theorem} via the dichotomy
