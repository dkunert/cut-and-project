...
both directions follows from \(s(\mathbb{Z}^2)\) being dense in
\(\mathbb{R}\) and \(W\) having nonempty interior: for every \(M > 0\)
there exists \((x,y) \in \mathbb{Z}^2\) with \(s(x,y) \in W\) and
\(|x| > M\), and \(\tilde p(x,y) = x + a y\) is unbounded as such
\(x\) is.  Its gap sequence is therefore well defined via the increasing
...
of the repository~\cite{Kunert2025} at commit \texttt{0b12c26}.}
\label{tab:lean}
\end{table}
