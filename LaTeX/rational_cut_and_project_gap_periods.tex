...
The algebraic core of both proofs has been formalised in Lean~4.  The
...
If \(D \nmid N\), then the multiset difference sequence has minimal
period \(\lambda = N\).
\end{proposition}
...
\[
    \delta^{(i+N)}
    =
    \tilde p_s^{(i+N+1)}-\tilde p_s^{(i+N)}
    =
    \tilde p_s^{(i+1)}-\tilde p_s^{(i)}
    =
    \delta^{(i)}
\]
for all \(i\).  Thus \(N\) is a period of the difference sequence, and hence
...
\]
for all \(i\).  Hence the entire multiset \(S\) is invariant under
...
define the \emph{set-valued difference sequence}
...
in~\eqref{eq:ND}, the set-valued difference sequence~\eqref{eq:delta_set}
has minimal period
...
\begin{proof}
Write \(N = qD + s\) with \(0 \le s < D\) as in §\ref{subsec:step5}, and
...
The formalisation comprises approximately 1{,}650 lines of verified code
...
\begin{enumerate}
    ...
    determine whether the same period formulas persist for arbitrary window
    offsets.
