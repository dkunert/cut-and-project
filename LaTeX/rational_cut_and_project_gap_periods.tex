...
the projected set \(\tilde p(A)\) is locally finite.  Unboundedness in
both directions follows from the identity
\[
    \tilde p(x,y) \;=\; x + a y
    \;=\; (1 + a^2)\,x + a\,s(x,y),
\]
combined with the boundedness of \(s\) on accepted points
(\(s(x,y) \in W\)): on \(A\) the term \(a\,s(x,y)\) is uniformly bounded,
so \(\tilde p(x,y) \to \pm\infty\) precisely as \(x \to \pm\infty\) along
accepted points.  The required existence of accepted points with
\(x \to \pm\infty\) follows from the density of
\(s(\mathbb{Z}^2)=\{n - a m : m, n \in \mathbb{Z}\}\) in \(\mathbb{R}\)
(Kronecker / Weyl equidistribution): for any \(M > 0\) and any
\(t \in W\) we can find \((m, n) \in \mathbb{Z}^2\) with \(m > M\)
(respectively \(m < -M\)) and \(n - a m\) arbitrarily close to \(t\),
hence inside \(W\); the lattice point \((m, n)\) is then in \(A\) with
\(|m|\) arbitrarily large.  Its gap sequence is therefore well defined
via the increasing \(\mathbb{Z}\)-indexed enumeration of \(\tilde p(A)\).
