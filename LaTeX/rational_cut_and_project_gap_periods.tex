...
order; we record this fact for the present strip-projection setting in
...
\begin{tikzpicture}[>=stealth, scale=0.78]
  ...
  \foreach \x/\y in {0/0, 0/1, 1/0, 1/1, 1/2, 1/3,
                     2/2, 2/3, 2/4, 2/5,
                     3/4, 3/5, 3/6, 3/7} {
...
\begin{itemize}
    \item 51{,}012 test cases with \(\alpha, \beta\) up to \({\sim}100{,}000\)
          ...
          see file \texttt{multiset\_and\_set\_new\_find\_patterns\_51012\_lines.csv}
    ...
    \item 477 cases at \((\alpha, \beta) = (1, 2)\) with rational \(\omega\)
          ...
          \texttt{multiset\_and\_set\_degenerate\_patterns\_5000\_lines.csv}
          in the repository's \texttt{tests/} folder.
\end{itemize}
All 51{,}489 cases produce exact matches with zero failures.  These
computations are used only as consistency checks; the proof below is
independent of the empirical verification.
\end{remark}
...
\(M\) and \(\lambda\) are periods, their nonnegative integer
...
The dichotomy \(N < D\) versus \(N \ge D\) has been verified against
empirical data in the repository's \texttt{tests/} folder.  The two CSV
files used for the multiset formula record both
\(\lambda_{\mathrm{multiset}}\) and \(\lambda_{\mathrm{set}}\) on each row:
\texttt{multiset\_and\_set\_new\_find\_patterns\_51012\_lines.csv} contributes
51{,}011 cases with \(N < D\) and one with \(N \ge D\), and
\texttt{multiset\_and\_set\_degenerate\_patterns\_5000\_lines.csv} adds
477 cases with \(N \ge D\) (and \(D \mid N\)).  In addition,
\texttt{set\_theorem\_balanced\_test.csv} provides 240 cases generated by
an independent Python implementation, balanced 120 / 120 between the two
regimes and spanning 25 distinct \((\alpha, \beta)\) pairs.  All cases
match the formula with zero failures.  These computations are consistency
checks and are not used in the proof.
...
Let \(a \in \mathbb{R} \setminus \mathbb{Q}\) and \(\omega > 0\), and consider
the analogous strip projection with line \(f(x) = a x\) and strip
\(\{(x,y) : a(x-\omega) \le y \le a x + \omega\}\).  Then the resulting
projected gap sequence has \emph{no} finite period.
\end{proposition}

\begin{proof}
Suppose, for contradiction, that the gap sequence has a finite period
\(\lambda \in \mathbb{N}\).  Then there exists \(\sigma > 0\) (the sum of
\(\lambda\) consecutive gaps) such that the projected set
\(S' \subset \mathbb{R}\) satisfies \(S' + \sigma = S'\).  Since \(S'\) is
the orthogonal projection of \(\mathbb{Z}^2 \cap \text{strip}\) onto the
line, this translational symmetry must lift to a lattice translation
\((x, y) \mapsto (x + m, y + n)\) with \((m, n) \in \mathbb{Z}^2 \setminus
\{(0, 0)\}\) that preserves the strip.

For the strip to be invariant under \((x, y) \mapsto (x + m, y + n)\), the
inequalities \(a(x - \omega) \le y \le a x + \omega\) must transform into
themselves; this forces
\[
    n \;=\; a \cdot m, \qquad m \ne 0.
\]
Hence \(a = n/m \in \mathbb{Q}\), contradicting the irrationality of \(a\).
\end{proof}

\begin{remark}[Period dichotomy]\label{rem:dichotomy}
Together, Corollary~\ref{cor:finite} and Proposition~\ref{prop:irrational}
...
with the boundary \(\mathbb{Q} \subset \mathbb{R}\).  The aperiodic side
is the classical setting of model sets and aperiodic order
