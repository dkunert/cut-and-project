...
The strip has bounded internal-coordinate width given by \(W\); since
the internal projection \(s\) is injective on \(\mathbb{Z}^2\) (by the
same irrationality argument used for \(\tilde p\) in Step~1 of the proof
of Proposition~\ref{prop:irrational} below) and \(\tilde p\) restricted
to the strip has finite fibres over any bounded interval --- the
preimage of such an interval intersected with the strip is a bounded
parallelogram in \(\mathbb{R}^2\) and hence contains only finitely many
lattice points --- the projected set \(\tilde p(A)\) is locally finite.
...
\(0 < |s(p_0, q_0)| < \varepsilon\).  Assuming \(\varepsilon \le 1\)
(which we ensure in the final choice below), the relation
\(s(p_0, q_0) = q_0 - a p_0\) with \(|s(p_0, q_0)| < \varepsilon \le 1\)
...
\(\varepsilon \le
 \min\!\bigl(1,\, c_0 \min(a\omega, \omega) / (|M| + 2 c_0 + 1)\bigr)\)
(still positive, depending on \(M\)) therefore satisfies both
\(\varepsilon \le 1\) (needed above to force \(p_0 \ne 0\)) and ensures
\(K \cdot |s(p_0, q_0)| < K \varepsilon \le \min(a\omega, \omega)\).
This matches the auxiliary parameter built with \(\min 1(\cdots)\) in
\texttt{exists\_scaled\_witness} (\texttt{Irrational.lean}).
...
\(z + v \in A\), hence \(A + v \subseteq A\).  For the reverse
inclusion, note that \(\Lambda + \sigma = \Lambda\) is equivalent to
\(\Lambda - \sigma = \Lambda\) (adding \(-\sigma\) to both sides of the
former yields the latter).  For any \(z \in A\), we have
\(\tilde p(z) - \sigma \in \Lambda - \sigma = \Lambda\), so there exists
\(z'' \in A\) with
\[
    \tilde p(z'') \;=\; \tilde p(z) - \sigma \;=\; \tilde p(z) - \tilde p(v)
    \;=\; \tilde p(z - v);
\]
injectivity of \(\tilde p\) on \(\mathbb{Z}^2\) (Step~1) gives
\(z'' = z - v\), hence \(z - v \in A\), i.e., \(A - v \subseteq A\).
Together these inclusions give \(A + v = A\).
