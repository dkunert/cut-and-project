...
with larger multiplicity form a proper cyclic interval after passing to the
\(m\)-stepping parametrization; its only translational symmetry is the identity,
...
\begin{equation}\label{eq:points}
    ...
    \right\}.
\end{equation}
The interval is closed: lattice points lying on either bounding line are
accepted.  This closed-boundary convention underlies the count
\(N = \lfloor\omega\alpha\rfloor + \lfloor\omega\beta\rfloor + 1\) of
admissible levels derived in Section~\ref{sec:mainresult}.
...
\begin{tikzpicture}[>=stealth, scale=0.78]
  ...
  \begin{scope}
    \clip (-0.3,-1.3) rectangle (3.6,7.6);
    \draw[blue!55!black, thick] (-0.3, 0.4) -- (3.6, 8.2);   % u(x)
    \draw[blue!55!black, thick] (-0.3,-2.6) -- (3.6, 5.2);   % l(x)
...
\begin{tikzpicture}[scale=1.0,
  ...
  \begin{scope}[yshift=2.6cm]
    ...
    \foreach \x/\m in {0/1, 1/1, 2/2, 3/1, 4/2} {
      \foreach \k in {1,...,\m} {
    ...
    \node[zerolabel] at (4,1.25) {\(\delta=0\)};
...
\[
    \alpha(x - \omega) \;\le\; \beta y \;\le\; \alpha x + \omega\beta,
\]
equivalently
...
\begin{equation}
    \mathcal{R} \;:=\;
    \bigl\{-\lfloor\omega\beta\rfloor,\;
    -\lfloor\omega\beta\rfloor + 1,\; \ldots,\;
    \lfloor\omega\alpha\rfloor\bigr\},
\end{equation}
using the identity \(\lceil{-x}\rceil = -\lfloor x \rfloor\).
Hence
\[
    |\mathcal{R}| =
    \lfloor\omega\alpha\rfloor + \lfloor\omega\beta\rfloor + 1 = N.
\]
...
strip condition~\eqref{eq:r_range}.  Since \(c_r \in P_r\), the progression
is nonempty, so every \(r \in \mathcal{R}\) is realized by an accepted
lattice point.  Thus \(P_r\) is exactly the set of
...
\end{equation}
where \(r_0 = -\lfloor\omega\beta\rfloor\).
...
\[
    \hat{\mu}(x) := \mu(mx), \qquad x \in \mathbb{Z}/D\mathbb{Z},
\]
...
\[
    H := \{x \in \mathbb{Z}/D\mathbb{Z} : \hat{\mu}(x)=q+1\}
\]
...
\]
Equivalently, the relabeled function \(\hat{\mu}\) satisfies
\[
    \hat{\mu}(x+\tau)=\hat{\mu}(x), \qquad \tau := m^{-1}\sigma \pmod D.
\]
...
We shall also use the standard density fact that
\[
    s(\mathbb{Z}^2)=\{n-a m : m, n\in\mathbb{Z}\}
\]
is dense in \(\mathbb{R}\) when \(a\) is irrational.  Indeed, the orbit
\(\{-a m \bmod 1 : m \in \mathbb{Z}\}\) is dense in \([0,1)\), and adding
integers yields density in all of \(\mathbb{R}\).
...
period \(\lambda \in \mathbb{N}\).  Let \(\Lambda_A := \tilde p(A) \subset
...
\(\Lambda_A + \sigma = \Lambda_A\).
Choose any \(z_0 \in A\); since \(\tilde p(z_0) + \sigma \in \Lambda_A\),
...
\(\tilde p(z) + \sigma \in \Lambda_A\), so there is \(z' \in A\) with
...
\[
    ...
    W \cap s(\mathbb{Z}^2).
\]
By the density observation above, \(s(\mathbb{Z}^2)\) is dense in
\(\mathbb{R}\).  If \(\tau > 0\), pick
...
comprise approximately 3{,}760 lines of verified code with no
\texttt{sorry} axioms, and are available in the accompanying code
repository~\cite{Kunert2025} under \texttt{Lean/CutAndProject/}.

\subsection{Architecture}\label{subsec:lean_arch}

The proof is structured in two parts, an abstract Lean part and a
concrete Lean part.  The abstract part proves
...
The concrete part constructs a concrete instance of this typeclass
...
Residue distribution (general) with multiplier
...
(both files in the accompanying code repository version submitted with the paper~\cite{Kunert2025}).}
\label{tab:lean}
\end{table}
...
the residual gap left by the earlier version of this development, in which the
abstract typeclass \texttt{GeometricProjection} was instantiated only for the
...
A companion to the present work gives a self-contained Lean~4 / Mathlib
formalization of the three-distance theorem itself, uniform in the rotation
number (rational and irrational alike)~\cite{KunertThreeGap2026}; its orbit of
fractional parts is, in the rational case, the same kind of projected point set
studied here, so the two formalizations are complementary.
...
\(p+q\)~\cite{BerstelEtAl2008}.  The corresponding mechanical
word -- in the convention where each letter encodes one unit step of
this path -- is then periodic with period \(p+q\) (the length of the
Christoffel word).  (We note that under
...
\subsection{Arithmetic discrete lines and digital geometry}

The strip condition~\eqref{eq:r_range},
\(-\omega\beta \le \alpha x - \beta y \le \omega\alpha\), has the same form as
an \emph{arithmetic discrete line} in the sense of
Reveill\`{e}s~\cite{Reveilles1991},
\[
    D(\alpha,\beta,\mu,W)
    = \bigl\{(x,y)\in\mathbb{Z}^2 : \mu \le \alpha x - \beta y < \mu + W\bigr\},
\]
here with \(\mu = -\omega\beta\) and arithmetic thickness
\(W = (\alpha+\beta)\,\omega\) (up to the choice of boundary convention).
This family of objects has been studied extensively in digital geometry, and
two of its periodicity results are directly comparable to ours:
\begin{itemize}
    \item the arithmetic discrete line \(D(\alpha,\beta,\mu,W)\) is invariant
          under translation by the direction vector \((\beta,\alpha)\)
          \cite{Reveilles1991}; and
    \item the chain code (Freeman word) of the digital straight line of
          rational slope \(\alpha/\beta\) is periodic, its period governed by
          the Stern--Brocot / continued-fraction structure of the slope
          \cite{LachaudSaid2013, BerstelEtAl2008}.
\end{itemize}

These results are nevertheless structurally distinct from
Theorem~\ref{thm:main}:
\begin{enumerate}
    \item \textbf{Different combinatorial object.} The digital-geometry
          results describe the chain code read in geometric order along the
          line, whereas Theorem~\ref{thm:main} concerns the sequence of
          distances between consecutive lattice points after projection and
          \emph{sorting} by \(\tilde{p} = \beta x + \alpha y\).
    \item \textbf{Dependence on \(\omega\).} The translation period of a
          discrete line depends only on its slope, whereas the projected gap
          period here depends on the strip width~\(\omega\) through the
          count~\(N\) of admissible levels.
    \item \textbf{No multiplicity convention.} Discrete lines are point sets;
          the multiset-versus-set distinction that is central to
          Section~\ref{sec:setvalued} has no counterpart there.
\end{enumerate}
The modulus \(D = \alpha^2+\beta^2\) of Theorem~\ref{thm:main} does occur in
this literature, but in a different role: it appears as a squared-radius /
curvature quantity in the arithmetic theory of discrete
circles~\cite{FiorioJametToutant2006}, not as the period modulus of a sorted
distance sequence.

\subsection{Cut-and-project sets and model sets}

The mainstream literature on model sets~\cite{BaakeGrimm2013,
BaakeGrimm2017, Senechal1995, Meyer1972, HaynesEtAl2016} focuses largely
on irrational slopes and aperiodic order.
Rational slopes yield periodic structures and are therefore often less
central from that perspective.  The closest work in the gap-statistics
direction is~\cite{HaynesEtAl2016}, which studies gap and patch-frequency
...
This exclusion is explicit in the standard pedagogical accounts of the area.
Haynes's lecture notes~\cite{HaynesNotes} single out the planar \(k=2\),
\(d=1\) cut-and-project construction with a \emph{rational} subspace as
``the degeneracy'': the resulting point set has a nontrivial group of periods
and is set aside as producing nothing not already understood, after which an
irrational slope is assumed throughout; the three-distance theorem is stated
there for general \(\alpha\), with its continued-fraction refinement given for
irrational rotations.  In Arnoux's chapter on Sturmian sequences in the Pytheas
Fogg volume~\cite{PytheasFogg2002}, a sequence is Sturmian precisely when it is
balanced and \emph{not} eventually periodic, so the rational, eventually
periodic rotation codings fall outside that framework, and no minimal-period
formula of the present kind is formulated there.  In neither reference does the modulus
\(D = \alpha^2+\beta^2\), nor the multiset/set period dichotomy, occur.  The
present formula records, with an explicit closed form and a machine-checked
proof of its residue-combinatorial core, precisely the case these standard
treatments set aside as degenerate.

Where the rational case is treated, the emphasis is on different invariants:
for instance, \cite{GaehlerHuntonKellendonk2013} computes the integral
\v{C}ech cohomology of the tiling spaces associated with rational
projection-method patterns, an algebraic-topological invariant unrelated to
the minimal period of the sorted gap sequence studied here.  The constant
\(D = \alpha^2+\beta^2\) itself does appear in this literature, but in an
enumerative role: in Baake and Zeiner's Chapter~3 of~\cite{BaakeGrimm2017} it is
the Gaussian-integer norm form that counts the similar sublattices and
coincidence-site lattices of \(\mathbb{Z}^2\), not the period modulus of a gap
sequence.

A more recent and active body of work on cut-and-project sets, principally by
Koivusalo and coauthors, develops quantitative invariants of these structures:
the patch-complexity growth exponent~\cite{KoivusaloWalton2021}, linear
repetitivity~\cite{KoivusaloWalton2020}, density
discrepancy~\cite{KoivusaloLagace2024}, and the substitutional structure of the
associated tilings~\cite{HarrissKoivusaloWalton2025}.  This work consistently
assumes the projected lattice to be dense (the slope totally irrational), so the
resulting patterns are aperiodic and possess no finite period; the invariants are
asymptotic growth rates governed by the Diophantine type of the slope, and
\(D = \alpha^2+\beta^2\) plays no role.  The closest in vocabulary is the
codimension-one linear-repetitivity criterion of~\cite{KoivusaloWalton2020}, which
involves a slope, an interval window, an integer defined by a lattice-membership
condition, and the distance-to-nearest-integer quantity
\(\lVert\alpha n+\beta\rVert\).  The resemblance is only verbal: the slope there is
irrational and the conclusion is a linear-repetitivity rate
\(\lVert\alpha n+\beta\rVert \ge c/n\) rather than the finite period of
Theorem~\ref{thm:main}, the relevant integer is a lattice-membership order rather
than the window count
\(N = \lfloor\omega\alpha\rfloor + \lfloor\omega\beta\rfloor + 1\), and
\(\alpha^2+\beta^2\) does not occur.  The substitution-theoretic
paper~\cite{HarrissKoivusaloWalton2025} treats the same planar \(k=2\), \(d=1\),
\(\mathbb{Z}^2\) cut-and-project setting (its general framework covers
higher-dimensional Euclidean schemes as well) and asks when an aperiodic set of
irrational slope is substitutional; the rational slope studied here is precisely the
periodic case those results exclude.
...
exploration, literature search, drafting, proof construction, and the creation of the
Lean formalization.
...
Michael Baake and Uwe Grimm (eds.),
...
Christophe Fiorio, Damien Jamet, and Jean-Luc Toutant,
``Discrete circles: an arithmetical approach with non-constant thickness,''
in \emph{Vision Geometry XIV}, Proceedings of SPIE, vol.~6066,
art.~60660C, 2006.
\textsc{doi}: \href{https://doi.org/10.1117/12.642976}{10.1117/12.642976}.

\bibitem{GaehlerHuntonKellendonk2013}
Franz G\"ahler, John Hunton, and Johannes Kellendonk,
``Integral cohomology of rational projection method patterns,''
\emph{Algebraic \& Geometric Topology}, vol.~13, no.~3, pp.~1661--1708, 2013.
\textsc{doi}: \href{https://doi.org/10.2140/agt.2013.13.1661}%
{10.2140/agt.2013.13.1661}.

\bibitem{HarrissKoivusaloWalton2025}
Edmund Harriss, Henna Koivusalo, and James J.\ Walton,
``When is a cut and project set substitutional?,''
preprint, 2025.
arXiv:\href{https://arxiv.org/abs/2512.13659}{2512.13659}.
...
\textsc{doi}: \href{https://doi.org/10.1017/S0305004116000128}%
{10.1017/S0305004116000128}.

\bibitem{HaynesNotes}
Alan Haynes,
\emph{Notes on Diophantine approximation and aperiodic order},
lecture notes, University of Houston, June~2016.
Available at \url{https://www.math.uh.edu/~haynes/notes.html}.

\bibitem{KoivusaloLagace2024}
Henna Koivusalo and Jean Lagac\'e, with an appendix by Michael Bj\"orklund and
Tobias Hartnick,
``Sharp density discrepancy for cut and project sets: an approach via lattice point
counting,''
\emph{Monatshefte f\"ur Mathematik}, vol.~208, no.~3, pp.~397--444, 2025.
\textsc{doi}: \href{https://doi.org/10.1007/s00605-025-02135-5}%
{10.1007/s00605-025-02135-5}.

\bibitem{KoivusaloWalton2020}
Henna Koivusalo and James J.\ Walton,
``Cut and project sets with polytopal window II: linear repetitivity,''
\emph{Transactions of the American Mathematical Society}, vol.~375, no.~7,
pp.~5097--5149, 2022.
...
\emph{Ergodic Theory and Dynamical Systems}, vol.~41, no.~5, pp.~1431--1463, 2021.
...
Jacques-Olivier Lachaud and Mouhammad Sa\"{i}d,
``Two efficient algorithms for computing the characteristics of a subsegment
of a digital straight line,''
\emph{Discrete Applied Mathematics}, vol.~161, no.~15, pp.~2293--2315, 2013.
\textsc{doi}: \href{https://doi.org/10.1016/j.dam.2012.08.038}%
{10.1016/j.dam.2012.08.038}.
...
edited by V.\ Berth\'{e}, S.\ Ferenczi, C.\ Mauduit, and A.\ Siegel,
Lecture Notes in Mathematics, vol.~1794, Springer, Berlin, 2002.
\textsc{doi}: \href{https://doi.org/10.1007/b13861}{10.1007/b13861}.

\bibitem{Reveilles1991}
Jean-Pierre Reveill\`{e}s,
\emph{G\'{e}om\'{e}trie discr\`{e}te, calcul en nombres entiers et
algorithmique},
Th\`{e}se d'\'{E}tat, Universit\'{e} Louis Pasteur, Strasbourg, 1991.
HAL: \href{https://theses.hal.science/tel-01279525}{tel-01279525}.
