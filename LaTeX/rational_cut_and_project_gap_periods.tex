...
\begin{tikzpicture}[>=stealth, scale=0.78]
  ...
  % projection of selected accepted points onto f
  % proj of (x,y) onto f(x)=2x is at ((x+2y)/5, 2(x+2y)/5)
  \draw[gray!65, thin] (0,1) -- (0.4, 0.8);
  \fill[red!70!black] (0.4, 0.8) circle (1.9pt);
  \draw[gray!65, thin] (1,3) -- (1.4, 2.8);
  \fill[red!70!black] (1.4, 2.8) circle (1.9pt);
  \draw[gray!65, thin] (2,3) -- (1.6, 3.2);
  \fill[red!70!black] (1.6, 3.2) circle (1.9pt);
  \draw[gray!65, thin] (3,5) -- (2.6, 5.2);
  \fill[red!70!black] (2.6, 5.2) circle (1.9pt);
...
\caption{Construction of the cut-and-project sequence for
  ...
  onto the line \(f(x)=(\alpha/\beta)x\) (red); four such projections are shown
  as red dots on \(f\) connected to the lattice point by a thin line.
...
\end{itemize}
All 51{,}480 cases produce exact matches with zero failures.  These
...
case split.  These computations are consistency checks and are not used
in the proof.
