...
Now fix the target \(M\) and shrink \(\varepsilon\) (depending on
\(M\)) so that the integer
\(K := \lceil M / \tilde p(p_0, q_0) \rceil + 1\) satisfies
\(K \cdot |s(p_0, q_0)| \le \min(a\omega, \omega)\).  Then
\(s(K p_0, K q_0) = K \cdot s(p_0, q_0) \in W\), so
