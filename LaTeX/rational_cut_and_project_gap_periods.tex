...
\begin{proof}
Define the physical and internal coordinates
\[
    \tilde p(x, y) \;:=\; x + a y,
    \qquad
    s(x, y) \;:=\; y - a x.
...
\(1/\sqrt{1 + a^2}\).  The strip condition is equivalent to
\[
    -a\omega \;\le\; s(x, y) \;\le\; \omega,
\]
so the accepted lattice points are
\[
...
\(x_1 - x_2 = -a\,(y_1 - y_2)\), which, since \(a\) is irrational and
\(x_i, y_i \in \mathbb{Z}\), forces \(y_1 = y_2\) and hence \(x_1 = x_2\).
...
\mathbb{R}\) be the projected set, and let \(\sigma > 0\) be the sum of
one full period of consecutive gaps; then \(\Lambda + \sigma = \Lambda\).
...
\(z + v \in A\).  Applying the same argument to the period \(-\sigma\)
yields the reverse inclusion, so \(A + v = A\).

\emph{Step 3: Internal density forces \(v = 0\).}\quad
Write \(v = (m, n)\) and let \(\tau := s(v) = n - a m\).  The invariance
\(A + v = A\) translates, in internal coordinates, into
\[
...
\(t + \tau > \omega\), contradicting the invariance.  If \(\tau < 0\),
an analogous choice near the lower endpoint \(-a\omega\) yields the same
contradiction.  Hence \(\tau = 0\), i.e.\ \(n = a m\).  Since \(a\) is
