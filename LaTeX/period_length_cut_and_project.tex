...
\title{Period Length of One-Dimensional\\ Cut-and-Project Sequences\\
...
\author{
    ...
    \and
    Gemini 3.1 Pro (High)\thanks{AI research assistant (Google). Derived and proved
    the formula for $\omega \ne 1$, formalised the proof in Lean~4, and
    drafted the initial version of this article.}
    \and
    Claude Opus 4.6\thanks{AI research assistant (Anthropic). Continued formalising
    the proof in Lean~4 and worked on the initial version of this article.}
}
\date{\today}
...
The proof proceeds by analysing the multiplicity function of residue
classes on $\mathbb{Z}/D\mathbb{Z}$: when $D \nmid N$ the ``heavy''
residue classes form a contiguous cyclic interval, whose trivial
stabiliser precludes any sub-periodicity.
The formula is additionally verified in over 51{,}000 computational test
cases spanning both the regime $N < D$ and $N \ge D$.
The algebraic core of the proof has been machine-checked in
Lean~4 with the Mathlib library (approximately 1{,}300 lines, no
\texttt{sorry} axioms).
As immediate corollaries, we obtain: finiteness of the period for all
real~$\omega\ge 0$; the proven result $\lambda = \alpha + \beta + 1$ at
...
11B25, 52C23, 11J71, 11A05, 68V20.

\medskip
\noindent\textbf{Keywords:}
cut-and-project sequences, period length, floor function, rational slope,
B\'{e}zout's identity, lattice projection, Lean~4 formalisation.
...
rational slopes and all nonnegative real window widths.
...
Section~\ref{sec:lean} describes the machine-checked formalisation in Lean~4.
...
$0 < s < D$, exactly $s$ residue classes are hit $q+1$ times and $D - s$
residue classes are hit $q$ times.
\end{lemma}
\begin{proof}
The map $r \mapsto mr \bmod D$ is a bijection on $\mathbb{Z}/D\mathbb{Z}$
...
After $q$ complete cycles ($qD$ values), all classes have been hit exactly
$q$ times.  The remaining $s = N - qD$ values ($0 < s < D$ since
$D \nmid N$) hit $s$ additional classes exactly once each.
...
\emph{multiplicity function} argument on $\mathbb{Z}/D\mathbb{Z}$.

Define the multiplicity function $\mu \colon \mathbb{Z}/D\mathbb{Z}
\to \mathbb{N}_0$ by
\[
    \mu(c) \;:=\; \bigl|\{r \in \mathcal{R} : c_r \equiv c \pmod{D}\}\bigr|.
\]
By Lemma~\ref{lem:nonuniform}, with $N = qD + s$ and $0 < s < D$,
the function $\mu$ takes exactly two values: $q+1$ on a set of $s$ residue
classes and $q$ on the remaining $D - s$ classes.
...
Since $m$ is invertible modulo $D$, this is just a relabelling of the same
data.  In this parametrisation, the ``heavy'' set
\[
...
$\tilde p_s^{(i+\lambda')} = \tilde p_s^{(i)} + \sigma$ for all $i$, so the
entire multiset $S$ is invariant under translation by $\sigma$.
On the other hand, $N$ consecutive gaps always sum to $D$, because every
interval $[t,t+D)$ contains exactly $N$ points of $S$ counted with
multiplicity.  Hence
\[
    k\sigma = D,
\]
...
But a proper nonempty cyclic interval in $\mathbb{Z}/D\mathbb{Z}$ has trivial
stabiliser: if $0<s<D$ and
\[
    H=\{x_0,x_0+1,\ldots,x_0+s-1\}\pmod D,
\]
then $H+\tau=H$ forces $\tau\equiv 0\pmod D$.
Indeed, for $1\le \tau \le D-1$, the translate $H+\tau$ has a different left
endpoint and cannot coincide with $H$ unless $H$ is either empty or all of
...
period.  Since we already know $\lambda \mid N$, it follows that $\lambda=N$.
\qed
...
all $\omega \in \mathbb{R}_{\ge 0}$, the period length~$\lambda$ is finite.
\end{corollary}
\begin{proof}
...
$\alpha, \beta \ge 1$ (because $\alpha(\alpha-1) + \beta(\beta-1) \ge 0$),
we have $N < D$, hence $D \nmid N$ and
...
dropped.  For $\alpha = 2$, $\beta = 1$, $\omega = 3/2$:
$N = \lfloor 3 \rfloor + \lfloor 3/2 \rfloor + 1 = 3 + 1 + 1 = 5 = D$.
...
arbitrarily small periods: $\lambda = N/D$ may be much less than
$\Lambda_{\alpha,\beta}$.
This occurs when the window width is tuned so that the number of tracks
is an exact multiple of $D = \alpha^2 + \beta^2$.
\end{remark}
...
The algebraic core of the proof has been fully formalised in
...
results of this paper and the Lean declarations in
\texttt{Basic.lean}.
...
\begin{tabular}{lll}
\hline
\textbf{Paper} & \textbf{Lean name} & \textbf{Line} \\
\hline
...
The formalised proof covers the full algebraic/combinatorial argument:
coprimality, residue distribution, cyclic interval structure, the
trivial stabiliser lemma, the generic minimality argument, the
degenerate case, and the final case dispatch.

The geometric construction (defining the cut-and-project set,
projecting lattice points, and sorting the multiset of
$\tilde{p}$-values) is modelled abstractly via the
\texttt{GeometricProjection} typeclass rather than formalised from
first principles.  Concretely, the sorted multiset is defined
combinatorially as $\tilde{p}^{(i)}_s := V(i \bmod N) + \lfloor i/N
\rfloor \cdot D$, where $V$ is the quantile function of the
cumulative residue distribution.  The bridge lemma
\texttt{count\_hits\_eq\_sorted\_count} verifies that this
...
research.
The AI model Gemini 3.1 Pro (High) (Google) derived and proved the formula
for $\omega \ne 1$, formalised the proof in Lean~4
(Section~\ref{sec:lean}), and drafted the initial version of this
article; it is listed as co-author accordingly.
The AI model Claude Opus 4.6 (Anthropic) continued formalising the proof
in Lean~4 and worked on the initial version of this article; it is
listed as co-author accordingly.
The AI model GPT-5.4 (OpenAI) reviewed a subsequent draft and
contributed the trivial-stabiliser argument for the contiguous heavy
set, as well as the observation that the proof extends to all real
$\omega \ge 0$.
