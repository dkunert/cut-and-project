...
collapses to $1$. The next section explains why this is an
\emph{algorithmic artifact} of the period-detection heuristic, not a
genuine mathematical disagreement between the set and multiset notions
of period.

\section*{Why $\lambda_{\text{set}} = 1$ is reported for this row}

The disagreement is fully explained by inspecting the dx-sequence the
algorithm operates on.

\paragraph{Parameters and window.}
For this row $\alpha = 68$, $\beta = 149$ (with $\gcd(\alpha,\beta)=1$),
$\gamma = 98{,}281$, $\delta = 139$ (with $\gcd(\gamma,\delta)=1$), and
$\omega = \gamma/\delta \approx 707.06$. The width of the cut window in
$y$ is
\[
...
raw projected values $\beta x + \alpha y$.
...
The reported $\lambda_{\text{set}} = 1$ is \emph{locally} correct — the
bulk of the sorted-and-collapsed difference sequence really is a
constant $1$ — but it does not reflect the global periodic structure of
the cut-and-project sequence. The full sequence of distinct gap values,
including the rare boundary patches at indices $[0, 5970]$ and
$[231{,}822, 237{,}812]$, has a non-trivial periodic structure whose
period coincides with the multiset period $153{,}431$. The set-collapse
step throws away the multiplicities (the number of lattice pairs
$(x,y)$ projecting to the same integer), and once the cut window is wide
enough that the projected values \emph{saturate} $\mathbb{Z}$ in the
bulk, the surviving SET-sequence is locally constant with no
information left for the heuristic to latch onto.

\paragraph{Consequences for the conjecture.}
Within the present implementation the row is not a true counterexample
to ``$\lambda_{\text{set}} = \lambda_{\text{multiset}}$''; it is a
diagnostic that for large $\omega$ the SET-based heuristic collapses to
the trivial period $1$ on the bulk before it can see the boundary
patches that carry the actual period. To produce a meaningful
$\lambda_{\text{set}}$ in the large-$\omega$ regime one would either
need to (i) discount the boundary effect explicitly, (ii) require the
detected period to occur multiple times across the array, or (iii) work
with a representation of the gap sequence that does not collapse
multiplicities in the first place — at which point the SET notion
reduces to the multiset one.
