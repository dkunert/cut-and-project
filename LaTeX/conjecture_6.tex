...
classes on $\mathbb{Z}/D\mathbb{Z}$: when $D \nmid N$ it has exactly two
transitions, which precludes any sub-periodicity.
The formula is additionally verified in over 32{,}000 computational test
...
Let $\alpha, \beta \in \mathbb{N}$ with $\gcd(\alpha, \beta) = 1$ and let
$\omega \in \mathbb{Q}_{\ge 0}$.  Define
...
\begin{itemize}
    ...
    \item 450 deliberately constructed cases in the degenerate regime
          $D \mid N$ (with $\alpha, \beta \le 10$ and $N$ up to
          ${\sim}500$), verifying the $\lambda = N/D$ branch.
\end{itemize}
All 51{,}462 cases produce exact matches with zero failures.
\end{remark}
...
The crucial observation is that the $s$ ``heavy'' classes (those with
multiplicity $q+1$) form a \emph{contiguous arc} on the circle
$\mathbb{Z}/D\mathbb{Z}$.
To see this, note that the $N$ values of $r$ form a contiguous block
$\{r_0, r_0+1, \ldots, r_0+N-1\}$, and the bijection $r \mapsto mr \bmod D$
maps this block to $N$ equally spaced points (step $m$) on the circle.
After $q$ complete orbits, the first $qD$ values cover every class exactly
$q$ times.  The remaining $s$ values $\{r_0+qD, \ldots, r_0+N-1\}$ map
to $s$ consecutive points (in the $m$-stepping order) on the circle, each
incrementing the multiplicity from $q$ to $q+1$.
Hence the multiplicity function has exactly \textbf{two transitions}
(``jumps'') on the circle: one from $q$ to $q+1$ and one from $q+1$ back
to~$q$.

Now suppose, for contradiction, that $\lambda' < N$ is a period, with
$k := N/\lambda' > 1$.  A sub-period $\lambda'$ in the gap sequence means
that the multiset of $N$ points on $\mathbb{Z}/D\mathbb{Z}$ is invariant
under a cyclic translation by some shift $\sigma \ne 0$ (namely, the sum
of the first $\lambda'$ gaps).

Concretely, since any interval of length $D$ in $\tilde{p}$-space contains
exactly $N$ points, the sum of $N$ consecutive gaps is always exactly~$D$.
A sub-period of length $\lambda'$ repeating $k$ times must therefore sum to
$\sigma = D/k$, requiring $k \mid D$.  The shifted gaps map the point
configuration onto itself, forcing $\mu(c) = \mu(c + \sigma)$ for all
$c \in \mathbb{Z}/D\mathbb{Z}$.

Such translational invariance requires $\mu$ to be periodic.  In the
$m$-stepping order, evaluating $\mu$ along the circle corresponds to
$f(x) = \mu(mx)$, which is thus invariant under a shift by $m^{-1}\sigma$.
Because $\gcd(m,D)=1$ and $\sigma$ has additive order $k$, the element
$m^{-1}\sigma$ also has additive order $k$ in $\mathbb{Z}/D\mathbb{Z}$.
The subgroup generated by this shift is the unique subgroup of size~$k$
(the multiples of $D/k$).  Therefore, the sequence of values of $\mu$ along
the $m$-circle consists of exactly $k$ identical repeating blocks.  The
pattern of jumps must thus repeat $k \ge 2$ times along the $m$-circle,
yielding at least $2k \ge 4$ transitions.

But we have established that $\mu$ has exactly $2$ transitions,
contradicting $2k \ge 4$.  Therefore no sub-period $\lambda' < N$ exists,
and $\lambda = N$.
\qed
...
The AI model Gemini 2.5 Pro (Google) reviewed a draft of this paper and
contributed the key insight for the minimality proof
(Section~\ref{subsec:step5}): the multiplicity function on
$\mathbb{Z}/D\mathbb{Z}$ has exactly two transitions, which precludes
sub-periodicities.  Gemini also identified errors in an earlier version
of Lemma~\ref{lem:nonuniform} and Corollary~\ref{cor:medium_omega}.
