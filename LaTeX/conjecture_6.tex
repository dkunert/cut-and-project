...
under a cyclic translation by some shift $\sigma \ne 0$ (namely, the shift
that maps the first block of $\lambda'$ gaps onto the second).
Concretely, the sorted differences repeat every $\lambda'$ steps, so the
residues of the $N$ points within each interval $[kD, (k+1)D)$ must be
identical when partitioned into consecutive blocks of size~$\lambda'$;
this forces $\mu(c) = \mu(c + \sigma)$ for all $c \in \mathbb{Z}/D\mathbb{Z}$.
Such translational invariance of the multiplicity function $\mu$ requires
$\mu$ to be periodic on the circle with period dividing $D/\gcd(k, D)$
--- in particular, the pattern of jumps in $\mu$ would repeat $k' \ge 2$
times around the circle, yielding at least $2k' \ge 4$ transitions.

But we have established that $\mu$ has exactly $2$ transitions.
Hence $k' = 1$, which means $\mu$ is constant --- but a constant $\mu$
requires $s = 0$, i.e.\ $D \mid N$, contradicting our hypothesis.
