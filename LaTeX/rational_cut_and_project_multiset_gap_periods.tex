...
sequence.  Since the minimal period divides every period, and we already know
that \(N\) is a period, it is enough to consider a proper period
\(\lambda'\) dividing \(N\).  Thus write
\[
    N = k\lambda'
\]
with \(k>1\).
...
Indeed, since \(0<s<D\), the interval \(H\) is uniquely determined by its
starting element in \(\mathbb{Z}/D\mathbb{Z}\); the translate \(H+\tau\)
starts at \(x_0+\tau\neq x_0\), so \(H+\tau\neq H\).
